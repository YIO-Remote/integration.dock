...
void DockPlugin::create(const QVariantMap &config, QObject *entities,
  ...
  connect(
      m_api, &YioAPIInterface::serviceDiscovered, context,
      [=](QMap<QString, QVariantMap> services) {
        timeOutTimer->stop();
        ...
        for (i = services.begin(); i != services.end(); i++) {
          ...
          conf.insert(
              "friendly_name",
              i.value().value("txt").toMap().value("FriendlyName").toString());

          Dock *db = new Dock(conf, i.value(), entities, notifications, api,
                              configObj, m_log);
  ...
  connect(timeOutTimer, &QTimer::timeout, this, [=]() {
    ...
    NotificationsInterface *m_notifications =
        qobject_cast<NotificationsInterface *>(notifications);
...
Dock::Dock(const QVariantMap &config, const QVariantMap &mdns,
  ...
  QObject::connect(m_websocketReconnect, SIGNAL(timeout()), this,
                   SLOT(onTimeout()));
  ...
  QObject::connect(m_heartbeatTimeoutTimer, &QTimer::timeout, this,
                   &Dock::onHeartbeatTimeout);
}
...
void Dock::onTimeout() {
  if (m_tries == 3) {
    ...
    QObject *param = this;
    m_notifications->add(
        true, tr("Cannot connect to ").append(friendlyName()).append("."),
        tr("Reconnect"),
        [](QObject *param) {
...
void Dock::webSocketSendCommand(const QString &domain, const QString &service,
...
void Dock::sendCommand(const QString &type, const QString &entity_id,
  ...
  if (type == "remote") {
    ...
    RemoteInterface *remoteInterface =
        static_cast<RemoteInterface *>(entity->getSpecificInterface());

...
void DockPlugin::create(const QVariantMap &config, EntitiesInterface *entities, NotificationsInterface *notifications,
    ...
    connect(timeOutTimer, &QTimer::timeout, this, [=]() {
        qCWarning(m_log) << "Cannot find any YIO Docks.";
        QMap<QObject *, QVariant> returnData;
...
void Dock::onTimeout() {
    if (m_tries == 3) {
        m_wsReconnectTimer->stop();
        qCCritical(m_log) << "Cannot connect to docking station: retried 3 times connecting to" << m_ip;
        ...
        qCDebug(m_log) << "Reconnection attempt" << m_tries + 1 << "to docking station:" << url;
...
void Dock::connect() {
    ...
    QString url = QString("ws://").append(m_ip).append(":946");
    qCDebug(m_log) << "Connecting to docking station:" << url;
    m_webSocket->open(QUrl(url));
}

void Dock::disconnect() {
    m_userDisconnect = true;
    qCDebug(m_log) << "Disconnecting from docking station";
